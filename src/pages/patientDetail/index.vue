<template>
  <div class="chat">
    <div class="header">
      <div class="nav">
        <div class="nav-item" :class="{'selected': currentIndex === 0}" @click="selectNav(0)">患者资料</div>
        <div class="nav-item" :class="{'selected': currentIndex === 1}" @click="selectNav(1)">购买的服务</div>
      </div>
    </div>
        ...
        <div class="base-wrap">
          <!-- <div class="vip-service">
            <img
              ...
              alt
            />
            <div @click="toService">查看购买的服务></div>
          </div>-->
          <div class="base-card" v-if="patientInfo">
            <!-- <img class="edit-btn" @click="editInfo" src="/static/images/archives_edit.png" alt /> -->
            <div class="base-info">
              <div
                class="img"
    ...
    <div class="patient-service" v-show="currentIndex === 1">
      <scroll-view style="height: 100%" scroll-y>
        <div class="base-wrap">
          <!-- <div class="service-list">
            ...
            >
              ...
              <div class="right">
                <div class="part-one">
                  <div class="validity" v-if="data.remainingUnits !== 0">剩余{{data.remainingUnits}}天</div>
                  <div class="validity" v-else-if="data.isProcessing">今日到期</div>
                  <div class="validity grey" v-else>已过期</div>
                  <div class="time" v-if="data.isProcessing">{{data.remainingDate}}到期</div>
                </div>
                <img
                  class="icon"
                  v-if="data.remainingUnits"
                  src="/static/images/icon_index.png"
                  alt
                />
              </div>
            </div>
          ...
          <div class="service-list">
             <div class="service-item" v-if="isShowIMService" @click="toChat">
                <div class="left">
                  <div class="service-name">健康顾问</div>
                </div>
                <div class="right">
                  <div class="part-one">
                    <div class="validity">进行中</div>
                  </div>
                  <img class="icon" src="/static/images/icon_index.png" alt="">
              </div>
             </div>
             <div class="service-item" @click="toAsk">
                <div class="left">
                  <div class="service-name">图文咨询</div>
                </div>
                <div class="right">
                  <div class="part-one">
                    <div class="validity">{{askCount}}次</div>
                  </div>
                  <img class="icon" src="/static/images/icon_index.png" alt="">
              </div>
             </div>
  ...
  methods: {
    ...
    getDetail() {
      this.httpFly.post({
        ...
        // this.tags = res.tags.filter(obj => obj.type === 'Custom').map(obj => obj.value)
    ...
    queryServiceContracts() {
      this.httpFly.post({
        ...
        this.serviceList = res.items ? res.items.map(obj => {
          let date = new Date()
    ...
    getConsultationSummary() {
      this.httpFly.post({
        healthRecordId: this.id
      }, '/servicepackage/api/v1/doctor/Consult/Ask/GetConsultationSummary', res => {
        if (res && res.totalCount) {
          this.askCount = res.totalCount
        }
      })
    },
    queryGroups() {
      this.httpFly.post({
        healthRecordIDs: [this.id]
      }, 'servicepackage/api/v1/doctor/IM/QueryGroups', res => {
        if (res && res.items && res.items.length) {
          let progress = res.items[0]['progress']
          this.isShowIMService = progress === 100 || progress === 500
          this.groupId = res.items[0]['groupID']
        }
      })
    },
    toAsk() {
      ...
      wx.navigateTo({
        url: '/pages/askList/main?healthRecordId=' + this.id
      })
    },
    /**
     * 附件查看
